...
- 지금 당장 Spec으로 만들기엔 이르거나 나중으로 미룬 항목은 `todo.md`에 적어둔다 (예: 향후 Spec 후보, 미룬 마이그레이션 등)
- Spec 진행 중 발견했지만 이번 범위는 아닌 이슈도 여기 기록
...
- 변경사항이 여러 항목이면 제목 아래 빈 줄 하나 두고 `- 항목` 형태 리스트로 상세 기술 가능
```
<Keyword>: <설명>
...
정답이 명확히 갈리지 않는 사소한 구현 디테일(뭘 골라도 크게 상관없는 것)까지 매번 물어볼 필요는 없음 — 이런 경우 합리적인 기본값으로 임의 선택하되, 마치 근거 있는 결정인 것처럼 제시하지 말고 "임의 선택 — 근거 없음" 같은 표시를 코드 주석/spec 문서/`todo.md` 중 적절한 곳에 남겨서 나중에 재검토 가능하게 할 것.
